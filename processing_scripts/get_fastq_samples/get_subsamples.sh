...
# - Lab 10 (mapped to Subsample 1)
# - Lab 20 (mapped to Subsample 3)
# - Lab 21 (mapped to Subsample 5)
# ==================================================================================================================================
...
# Iterate to create the output directory to store the each subsamples FASTQ files
...
for sample in "${samples[@]}"; do
    ...
    CSV_FILE="${SAMPLE_DIR}/../samplesheet/samplesheet.csv" # Define the path to the input .csv file with sample information (lab and number of sequenced bases)
    TXT_FILE="${SAMPLE_DIR}/../samplesheet/samplesheet.txt" # Define the path to the input .csv file with sample information (lab and number of sequenced bases)

    # Convert the .csv samplesheet with information from each sample to a .txt file necessary to access information
    ...
    for lab in "${labs[@]}"; do
        if [ "$lab" == "10" ]; then base_folder=1; fi # Define Subsample 1, corresponding to FASTQ files for samples B, D and E with higher number of sequenced bases in sequencing lab 10
        if [ "$lab" == "20" ]; then base_folder=3; fi # Define Subsample 3, corresponding to FASTQ files for samples B, D and E with higher number of sequenced bases in sequencing lab 20
        if [ "$lab" == "21" ]; then base_folder=5; fi # Define Subsample 5, corresponding to FASTQ files for samples B, D and E with higher number of sequenced bases in sequencing lab 21
